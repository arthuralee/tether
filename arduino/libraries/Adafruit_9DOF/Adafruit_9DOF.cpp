...
bool Adafruit_9DOF::accelGetOrientation(sensors_event_t *event, sensors_vec_t *orientation)
{
  ...
  if(event->acceleration.z < 0){
    if(orientation->roll > 0) orientation->roll = 180 - orientation->roll;
    if(orientation->roll < 0) orientation->roll = -180 - orientation->roll;
  }
...
bool Adafruit_9DOF::fusionGetOrientation(sensors_event_t *accel_event, sensors_event_t *mag_event, sensors_vec_t *orientation)
{
  ...
  orig_roll = (float)atan2(accel_event->acceleration.y, accel_event->acceleration.z);
  orientation->roll = (float)atan(accel_event->acceleration.y/ sqrt(accel_event->acceleration.x*accel_event->acceleration.x +\
  ...
  if (accel_event->acceleration.y * sin(orientation->roll) + accel_event->acceleration.z * cos(orientation->roll) == 0)
    orig_pitch = accel_event->acceleration.x > 0 ? (PI_F / 2) : (-PI_F / 2);
  else
    orig_pitch = (float)atan(-accel_event->acceleration.x/ (accel_event->acceleration.y * sin(orientation->roll) + \
