/**
 * @file test549.cpp
 * @brief Apriltags to TCP socket
 * @author: Arthur Lee
 *
 * Adapted from original apriltags demo
 */
...
const string intro = "\n"
    "April tags to socket\n"
    "Apriltags library (C) 2012-2014 Massachusetts Institute of Technology\n"
    "Arthur Lee\n"
    "\n";
...
public:
  ...
  void parseOptions(int argc, char* argv[]) {
    int c;
    while ((c = getopt(argc, argv, ":h?dtC:F:H:S:W:E:G:B:D:")) != -1) {
  ...
  void processImage(cv::Mat& image, cv::Mat& image_gray, TCPStream *stream) {
  ...
  void loop(TCPStream *stream) {
    ...
    while (true) {
      ...
      processImage(image, image_gray, stream);
...
int main(int argc, char* argv[]) {
  ...
  TCPAcceptor* acceptor = new TCPAcceptor(8080);
